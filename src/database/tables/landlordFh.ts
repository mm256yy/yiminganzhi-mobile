...
export const LandlordTableNameFh = 'landlordFh'

export const LandlordDDLFh = `
create table if not exists ${LandlordTableNameFh} (
...
// 需要更新的字段定义 和 字段赋值
export const landlordFieldsFh = `'uid','id','padStatus','doorNo','type','name','reportStatus','reportDate','reportUserDate','reportUser','reportUserName','reviewReportStatus','reviewReportDate','reviewReportUser','signStatus','signDate','areaCode','townCode','villageCode','warnStatus','currentProgress','virutalVillageCode','content','longitude','latitude','card','updatedDate','isPadDelete','houseAllStatus','isDelete'`
// status 字段为 当前数据是否有变更的状态
export const getLandlordValuesFh = (item: LandlordType, status: 'default' | 'modify') =>
...
// 获取 更新的sql值
export const getLandlordSqlValuesFh = (data: LandlordType) =>
