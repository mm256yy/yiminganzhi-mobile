  ...
  <view class="select-wrap">
    ...
    <view class="content show">
      ...
      <view class="body">
        ...
        <view class="line" v-if="currentNatural && currentNatural.length" />
        <view class="select-item" v-if="currentNatural && currentNatural.length">
          <view class="select-title">自然村:</view>
          <view class="select-cont">
            <view
              class="common-item"
...
watch(
  ...
  (val) => {
    if (val && val.length) {
      ...
      const villageList = townList.find((item: any) => item.code === val[1]).children || []
      currentVillage.value = villageList
      currentNatural.value = villageList.find((item: any) => item.code === val[2]).children || []
    }
  },
  {
    immediate: true,
...
const naturalClick = (item: any) => {
  const condition = currentSelect.value[3] && currentSelect.value[3] === item.code
  currentSelect.value[3] = condition ? '' : item.code
  title.value[3] = condition ? '' : item.name
}
