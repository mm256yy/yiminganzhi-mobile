  ...
  <view>
    ...
    <view class="arch-box">
      <view class="arch-item">
        <view class="arch-label"><text class="red">*</text>完成时间：</view>
...
interface PropsType {
  ...
  immigrantHouseEmpty: Partial<ImmigrantHouseEmptyType> | undefined
}

const props = defineProps<PropsType>()
const emit = defineEmits(['submit'])
const text = ref<string>('本户房屋已腾空完毕，同意交付给工程建设指挥部处理。')
...
watch(
  ...
  (val) => {
    if (val) {
      // 基本信息
      const { houseEmptyOpinion, houseEmptyDate } = val

      if (houseEmptyOpinion) {
        text.value = houseEmptyOpinion
      }
      if (houseEmptyDate) {
        date.value = dayjs(houseEmptyDate).format('YYYY-MM-DD')
      }
    }
  },
...
const submit = async () => {
  ...
  const params: Partial<ImmigrantHouseEmptyType> = {
    houseEmptyOpinion: text.value,
    houseEmptyDate: dayjs(date.value).toString(),
    isHouseEmpty: '1'
  }
