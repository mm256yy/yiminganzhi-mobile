...
import { addInfrastructureVcApi, updateInfrastructureVcApi, getEvaLandlordItemApi } from '@/service'
...
const submit = () => {
  ...
  if (type === 'add') {
    addInfrastructureVcApi(uid, params)
      .then((res) => {
        if (res) {
    ...
    updateInfrastructureVcApi(uid, params)
      .then((res) => {
        if (res) {
          showToast(SUCCESS_MSG)
          routerBack()
