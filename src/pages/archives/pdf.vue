				...
				<uni-row class="m-b-10">
					...
					<uni-col :span="12">
						<view class="col">
							...
							<view class="content">
								{{ dataInfo.demographicList ? formatStr(dataInfo.demographicList.length) : '-' }}
							</view>
						</view>
					</uni-col>
				</uni-row>
		...
		sumball() {
			return this.tableData.reduce((totalPrice, currentItem) => {
				if (currentItem.isUpdate == '0') {
					totalPrice += this.formatStr(currentItem.totalPrice) == '-' ? 0 : this.formatStr(currentItem.totalPrice)
